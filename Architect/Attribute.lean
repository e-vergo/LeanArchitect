...
/-! ## Highlighting Capture Linter

A linter that captures SubVerso highlighting for declarations with `@[blueprint]`.
Linters run after command elaboration while info trees are still available,
making this the ideal hook point for capturing semantic highlighting.
-/

open Command in
/-- Collect all syntax nodes matching a predicate. -/
private partial def collectSyntax (stx : Syntax) (p : Syntax → Bool) : Array Syntax :=
  let rec go (stx : Syntax) (acc : Array Syntax) : Array Syntax :=
    let acc := if p stx then acc.push stx else acc
    match stx with
    | .node _ _ args => args.foldl (init := acc) fun acc child => go child acc
    | _ => acc
  go stx #[]

open Command in
/-- Extract declaration names from a command syntax.
    Handles declarations, mutual blocks, and other definition forms. -/
private def getDeclNamesFromSyntax (stx : Syntax) : Array Name := Id.run do
  let mut names := #[]
  -- Find all declId nodes in the syntax tree
  let declIds := collectSyntax stx (·.isOfKind ``Lean.Parser.Command.declId)
  for declId in declIds do
    if let some idStx := declId[0]? then
      if idStx.isIdent then
        names := names.push idStx.getId
  return names

open Command in
/-- Linter that captures highlighting for blueprint declarations.

This linter runs after each command is elaborated. It:
1. Checks if the command defines declarations with `@[blueprint]` attribute
2. If so, captures SubVerso highlighting while info trees are still available
3. Stores the highlighting in the environment extension for later export
-/
def blueprintHighlightingLinter : Linter where
  name := `Architect.blueprintHighlightingLinter
  run := fun stx => do
